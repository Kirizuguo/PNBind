...
class Self_Attention(nn.Module):
    def __init__(self, hidden_size, num_attention_heads=4, num_neighbor=30, drop_rate=0):
        ...
        self.li3 = nn.Linear(hidden_size, hidden_size)
    ...
    def forward(self,q,k,v,attention_mask=None,attention_weight=None,use_top=True):
        ...
        # q = self.act(self.li1(q))
        # k = self.act(self.li2(k))
        # v = self.act(self.li3(v))
        ...
        outputs = torch.matmul(attention_probs.squeeze(0), v)
...
class GTM(nn.Module):
    def __init__(self, protein_in_dim=74, protein_out_dim=64, target_dim=2, 
                 fc_layer_num=2, atten_layer_num=2, atten_head=4, num_neighbor=30, 
                 drop_rate1=0.2, drop_rate2=0):
        super().__init__()
    ...
    def forward(self, point_graph):
        protein_node_features, hmm, secondary, edge_index, pos = point_graph.x, point_graph.hmm, point_graph.secondary, point_graph.edge_index, point_graph.pos
        pssm = point_graph.pssm

        protein_node_features = torch.cat((protein_node_features, hmm, secondary, pssm), dim=1)

        protein_masks = np.ones(protein_node_features.shape[0])
        protein_masks = torch.tensor(protein_masks, dtype = torch.long).unsqueeze(0).to(protein_node_features.device)
        protein_dist_matrix = torch.cdist(pos, pos)

        protein_node_features = protein_node_features.to(torch.float32)
        ...
        for layer in self.layers:
            protein_embedding = layer(protein_embedding, protein_embedding, protein_embedding, protein_masks, dist_weight) #.squeeze(1)#,dist_weight.unsqueeze(1)

        y = self.logit(protein_embedding)
...
def predict_one_protein(data_path, model_path, ID, msa):
    ...
    models = []
    for fold in range(5):
        model = GTM(node_dim, hidden_unit, class_num, fc_layer, self_atten_layer, attention_heads, 
                    num_neighbor, fc_dropout, attention_dropout)
