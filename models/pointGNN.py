...
class PointGNNModel(nn.Module):
    def __init__(self, in_features=1024, hidden_features=300, num_classes=2,n_layers=4):
        super(PointGNNModel, self).__init__()
        self.embed = nn.Sequential(nn.Linear(in_features, 128),
                                   nn.ReLU(),
    ...
    def forward(self, point_graph):
        x, pos, edge_index = point_graph.x, point_graph.pos, point_graph.edge_index
